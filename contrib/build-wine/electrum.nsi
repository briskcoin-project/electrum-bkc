...
;Variables
  ...
  !define PRODUCT_NAME "Electrum Briskcoin"
  !define PRODUCT_WEB_SITE "https://github.com/briskcoin-project/electrum-bkc"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-bkc-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\electrum-bkc\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-bkc\*.*"
  File "..\..\electrum-bkc\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bkc-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bkc-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-bkc-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-bkc-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-bkc-${PRODUCT_VERSION}.exe" 0


  ;Links briskcoin: and lightning: URIs to Electrum
  WriteRegStr HKCU "Software\Classes\briskcoin" "" "URL:briskcoin Protocol"
  WriteRegStr HKCU "Software\Classes\briskcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\briskcoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\briskcoin\shell\open\command" "" "$\"$INSTDIR\electrum-bkc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-bkc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\briskcoin"
