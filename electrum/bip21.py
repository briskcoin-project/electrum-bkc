...
BITCOIN_BIP21_URI_SCHEME = 'briskcoin'
...
def parse_bip21_URI(uri: str) -> dict:
    ...
    if ':' not in uri:
        if not bitcoin.is_address(uri):
            raise InvalidBitcoinURI("Not a briskcoin address")
        return {'address': uri}
    ...
    if address:
        if not bitcoin.is_address(address):
            raise InvalidBitcoinURI(f"Invalid briskcoin address: {address}")
        out['address'] = address
    if 'amount' in out:
        am = out['amount']
        try:
            ...
            if amount > TOTAL_COIN_SUPPLY_LIMIT_IN_BTC * COIN or amount <= 0:
                raise InvalidBitcoinURI(f"amount is out-of-bounds: {amount!r} BKC")
